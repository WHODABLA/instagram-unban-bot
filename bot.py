...
Instagram checking: uses a logged-in session via instagrapi (see
instagram_login.py) rather than anonymous requests, since Instagram
aggressively rate-limits/blocks anonymous checks from datacenter IPs.
...
def _fix_b64_padding(s: str) -> str:
    """Strip stray whitespace/newlines from a pasted base64 value and
    restore correct padding — mobile copy/paste often mangles both."""
    s = "".join(s.split())
...
def init_instagram_client():
    if not IG_SESSION_B64:
        raise SystemExit(
            "IG_SESSION_B64 not set. Run instagram_login.py locally first, "
            "then set IG_SESSION_B64 on Render. See README.md."
        )
    try:
        cleaned = _fix_b64_padding(IG_SESSION_B64)
        settings = json.loads(base64.b64decode(cleaned).decode("utf-8"))
    except Exception as e:
        raise SystemExit(
            f"IG_SESSION_B64 could not be decoded ({type(e).__name__}: {e}). "
            "This usually means the value got corrupted during copy/paste — "
            "re-copy the FULL contents of ig_session_b64.txt (select-all, not "
            "a manual drag-select) and re-paste it into Render's Environment tab."
        )
    ...
    try:
        ...
        print("Instagram session loaded and verified.")
    except LoginRequired:
        if not (IG_USERNAME and IG_PASSWORD):
            raise SystemExit(
                "Instagram session expired and IG_USERNAME/IG_PASSWORD are not "
                "set for automatic relogin. Regenerate the session with "
                "instagram_login.py, or set IG_USERNAME/IG_PASSWORD as a fallback."
            )
        print("Instagram session expired — attempting relogin with saved credentials...")
        ig_client.set_settings(settings)
        ig_client.login(IG_USERNAME, IG_PASSWORD, relogin=True)
        print("Instagram relogin succeeded.")
...
# ---------- Instagram status check (logged-in via instagrapi) ----------

def _sync_check_instagram(username: str):
    """Blocking call — always run this through asyncio.to_thread()."""
    try:
        user = ig_client.user_info_by_username_v1(username)
    except UserNotFound:
        return None
    except LoginRequired:
        if not (IG_USERNAME and IG_PASSWORD):
            print("Instagram session expired mid-run and no fallback credentials set.")
            return None
        try:
            ig_client.login(IG_USERNAME, IG_PASSWORD, relogin=True)
            user = ig_client.user_info_by_username_v1(username)
        except Exception as e:
            print(f"Relogin failed: {type(e).__name__}: {e}")
            return None
    except Exception as e:
    ...
    return {
        ...
        "profile_pic_url": str(user.profile_pic_url_hd or user.profile_pic_url or ""),
...
async def checknow(interaction: discord.Interaction, username: str):
    ...
    try:
        user = await asyncio.to_thread(ig_client.user_info_by_username_v1, username)
        await interaction.followup.send(
            f"✅ Live — Followers: {user.follower_count:,} | Posts: {user.media_count:,}"
        )
    except UserNotFound:
        await interaction.followup.send(
            "❌ UserNotFound — account is banned/suspended/doesn't exist "
            "(would be treated as still banned)."
        )
    except Exception as e:
