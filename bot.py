...
async def fetch_ig_session_from_d1() -> dict:
    config = await api_get_config()
    session_json = config.get("ig_session")
    if not session_json:
        raise SystemExit(
...
def init_instagram_client(settings: dict):
    ...
    try:
        ...
        if not (IG_USERNAME and IG_PASSWORD):
            raise SystemExit(
                ...
                "set for automatic relogin. Re-run instagram_login.py, or set "
                "IG_USERNAME/IG_PASSWORD as a fallback."
            )
...
if __name__ == "__main__":
    ...
    ig_settings = asyncio.run(fetch_ig_session_from_d1())
    init_instagram_client(ig_settings)
    start_keep_alive()
