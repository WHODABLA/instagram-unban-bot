...
async def on_app_command_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    # Without this handler, an exception inside a command just crashes silently
    # and Discord shows "The application did not respond" with no clue why.
...
async def checknow(interaction: discord.Interaction, username: str):
    ...
    async with aiohttp.ClientSession() as session:
        try:
            ...
            await interaction.followup.send(f"❌ Request failed entirely: `{type(e).__name__}: {e}`")
            return
    ...
    result_lines = [
        ...
        f"**Parsed as live?** {'✅ yes' if info else '❌ no (would be treated as still banned)'}",
    ]
    if info:
        result_lines.append(f"Followers: {info['followers']:,} | Posts: {info['posts']:,}")
    else:
        result_lines.append(f"**Raw response snippet:**\n```{body_text[:300]}```")
